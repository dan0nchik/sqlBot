...
updater = Updater(
    token='1139407419:AAEgdKd38btTS0VIkUVA-Yld968KHc_D4yk', use_context=True)
...
def start(update, context):
    ...
    update.message.reply_text(
        f"Бип Боп. Привет, <b>{user.first_name}!</b>"
...
def askSQLCommand(update, context):
    update.message.reply_text(
        "Введи команду SQLLite которая тебе нужна, и я пришлю тебе ссылку с ней.\n"
...
def getDBName(update, context):
    update.message.reply_text(
        "Ну что, создадим базу? А как она будет называться?")
    return 2
...
def column(update, context):
    ...
    update.message.reply_text(
        f"А как будет называться колонка? (Она у нас будет одна, больше делать мне лень)"
    )
    return 3
...
def columnType(update, context):
    ...
    update.message.reply_text(
        ...
        reply_markup=ReplyKeyboardMarkup(
            replies, one_time_keyboard=True, resize_keyboard=True))
    return 4
...
def createDBUserData(update, context):
    ...
    update.message.reply_text(
        "Обрабатываю твою информацию...", reply_markup=ReplyKeyboardRemove())
    user = update.message.from_user
    ...
    path = os.getcwd() + r"/{}".format(folder) + r"/{}.db".format(
    ...
    context.bot.send_document(
        chat_id=update.effective_chat.id, document=open(path, 'rb'))
    return ConversationHandler.END
    ...
    try:
        ...
        if connection:
            ...
            if cursor.fetchone()[0] == 1:
                ...
                cursor.execute(
                    """CREATE TABLE users (name TEXT, surname TEXT, username TEXT PRIMARY KEY);"""
                )
...
def startFillingTable(update, context):
    ...
    update.message.reply_text(
        "Внимание! Если ты еще не создал таблицы - тебе придется остановить меня :("
        " В другом случае, какую таблицу ты хочешь выбрать?",
        reply_markup=ReplyKeyboardMarkup(
            tables[0], one_time_keyboard=True, resize_keyboard=True))
    return 1
...
def getTableNameToFill(update, context):
    ...
    update.message.reply_text(
        "ОК, а какую колонку мы будем заполнять?",
    ...
    if folder is None:
        ...
        if folder is None:
            folder = user.first_name
    path = os.getcwd() + r"/{}".format(folder) + r"/{}".format(
        context.user_data['table'])
    connection = sqlite3.connect(path)
    cursor = connection.cursor()
    cursor.execute("""SELECT * FROM {};""".format(
        context.user_data['table'][:-3]))
    names = [[list(map(lambda x: x[0],
                       cursor.description))]]  # массив из колонок в таблице
    names[0][0].pop(0)  # удаляем id во избежание конфликтов
    update.message.reply_text(
        "Вот колонки",
        reply_markup=ReplyKeyboardMarkup(names[0], one_time_keyboard=True))
    return 2


def getColumnNameToFill(update, context):
    update.message.reply_text(
        "ОК, записал.", reply_markup=ReplyKeyboardRemove())
    ...
    cursor = connection.cursor()
    cursor.execute(
        f"""PRAGMA TABLE_INFO ({context.user_data['table'][:-3]}) ;""")
    ...
    update.message.reply_text(
        f"Внимание!!! У этой колонки тип {colType}. Это значит, что ты можешь вводить "
        f"значения "
...
def fillTableWithUserData(update, context):
    ...
    if context.user_data[
            'type'] == 'TEXT':  # тут кавычки добавляются так как TEXT
        cursor.execute(
            f"""INSERT INTO {context.user_data['table'][:-3]} ({context.user_data['column']}) VALUES ('{context.user_data['value']}');"""
        )
        connection.commit()
...
def newColumnName(update, context):
    context.user_data['table'] = update.message.text
    update.message.reply_text(
        "А как будет называться колонка?", reply_markup=ReplyKeyboardRemove())
    return 2
...
def alterTable(update, context):
    ...
    if folder is None:
        ...
        if folder is None:
            folder = user.first_name
    path = os.getcwd() + r"/{0}/{1}".format(folder,context.user_data['table'])
    connection = sqlite3.connect(path)
    cursor = connection.cursor()
...
def cancel(update, context):
    print("cancel")
    update.message.reply_text(
        "Бип Бип, прерываю диалог. Пиши /help для других команд")
    return ConversationHandler.END
...
if __name__ == '__main__':
    ...
    createDatabaseConvHandler = ConversationHandler(
        ...
        states={
            ...
            4: [
                MessageHandler(
                    Filters.text, createDBUserData, pass_user_data=True)
            ]
        },
        fallbacks=[])
    ...
    searchSQLCommandsConvHandler = ConversationHandler(
        ...
        states={
            1: [
                MessageHandler(
                    Filters.text, sendLinkToSQLCommand, pass_user_data=True)
            ]
        },
        fallbacks=[])
    fillTableConvHandler = ConversationHandler(
        ...
        states={
            1: [
                MessageHandler(
                    Filters.text, getTableNameToFill, pass_user_data=True)
            ],
            2: [
                MessageHandler(
                    Filters.text, getColumnNameToFill, pass_user_data=True)
            ],
            3: [
                MessageHandler(
                    Filters.text, fillTableWithUserData, pass_user_data=True)
            ]
        },
    ...
    alterTableConvHandler = ConversationHandler(
        ...
        states={
            1:
            [MessageHandler(Filters.text, newColumnName, pass_user_data=True)],
            2:
            [MessageHandler(Filters.text, newColumnType, pass_user_data=True)],
